...
char *BloomFilter_Package(BloomFilter *bf,char *disease){
    char *msg=malloc(strlen(disease)+2);
    strcpy(msg,disease);
    strcat(msg,"\n");
    char value[12];
    char pos[12];

    for (int i=0;i < bf->Bloomsize ;i++){
        memset(value,0,12);
        memset(pos,0,12);
        if(bf->Filter[i]!=0){
            sprintf(value,"%d\n",bf->Filter[i]);
            sprintf(pos,"%d\n",i);
            msg=realloc(msg,strlen(msg)+strlen(value)+strlen(pos)+1);
            strcat(msg,value);
            strcat(msg,pos);
        }
    }
    msg = realloc(msg,strlen(msg)+strlen("Fin\n")+1);
    strcat(msg,"Fin\n");
    return msg;
}
