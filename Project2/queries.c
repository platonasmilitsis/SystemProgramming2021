...
int monitor_commands(HashTable *ht,VirusTable *vt,BstList *bst_l,int rfd,int wfd,int buffer_size){
    ...
    char *command = read_from_pipe(rfd,buffer_size);
    ...
    if(strcmp(type,"/travelRequest")==0){
        monitor_travelRequest(l,ht,vt,bst_l,rfd,wfd,buffer_size);
    }
    else if(strcmp(type,"/searchVaccinationStatus")==0){
        monitor_searchVaccination(l,ht,vt,bst_l,rfd,wfd,buffer_size);
    }
    else{
        printf("\nERROR\n\n");
    }
...
int monitor_travelRequest(LinkedList *l,HashTable *ht,VirusTable *vt,BstList *bst_l,int rfd,int wfd,int buffer_size){
    ...
    write_to_pipe(wfd,msg,buffer_size);
    free(msg);
